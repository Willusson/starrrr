...
(function () {
  ...
  function updateTextParallax(x, y) {
    if (!starMark) starMark = document.querySelector('.star-mark');
    if (!starMark) return;
    /* hard cap so tilt can never push the word out of frame */
    var px = clamp(x, -14, 14), py = clamp(y, -14, 14);
  ...
  function render(t) {
    ...
    /* gentle depth parallax: nebula (far) < field < star < title, easing back */
    var ndx = -pointer.x * minSide * 0.012;
    var ndy = -pointer.y * minSide * 0.012;
    var pdx = -pointer.x * minSide * 0.022;
    var pdy = -pointer.y * minSide * 0.022;
    var sdx = -pointer.x * minSide * 0.034;
    var sdy = -pointer.y * minSide * 0.034;
  ...
  function bindTilt() {
    ...
    window.addEventListener('deviceorientation', function (e) {
      ...
      pointer.tx = clamp(g / 45, -1, 1) * 0.5;
      pointer.ty = clamp((b - 45) / 45, -1, 1) * 0.5;
    }, { passive: true });
  }
  ...
  function onStarTouch(e) {
    if (e.type === 'pointerdown' && e.pointerType === 'touch') return; /* touchstart covers it */
    var t = (e.changedTouches && e.changedTouches[0]) || e;
    if (!hitStar(t.clientX, t.clientY)) return;
  ...
  function bindTouch() {
    canvas.addEventListener('touchstart', onStarTouch, { passive: false });
    canvas.addEventListener('pointerdown', onStarTouch);
